...
interface BodegonProductsGridProps {
  products: Array<{
    id: string
    name: string
    sku?: string
...
export function BodegonProductsGrid({ products, onProductDeleted, categories = [], subcategories = [] }: BodegonProductsGridProps) {
  const router = useRouter()
  const [showDeleteModal, setShowDeleteModal] = useState(false)
  const [selectedProduct, setSelectedProduct] = useState<BodegonProduct | null>(null)

  const handleEdit = (productId: string) => {
    router.push(`/admin/bodegones/productos/${productId}/editar`)
  }
              ...
              <CardContent className="p-3">
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  ...
                  {!categoryName && !subcategoryName && (
                    <Badge 
                      variant="secondary" 
                      className="text-xs px-2 py-1 bg-gray-100 text-gray-600 hover:bg-gray-100"
                    >
                      Sin categoría
                    </Badge>
                  )}
